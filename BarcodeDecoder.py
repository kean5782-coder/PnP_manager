...
class BarcodeDecoderApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Расшифровка кода компонента")
    ...
    def has_cyrillic(self, text):
        """Возвращает True, если в тексте есть хотя бы одна русская буква."""
        return bool(re.search(r'[а-яА-ЯЁё]', text))

    def update_status_for_text(self, text):
        """Обновляет строку состояния в зависимости от наличия кириллицы."""
        if self.has_cyrillic(text):
            self.status_var.set("⚠️ Для нормальной работы приложения смените раскладку на английскую.")
            return True
        else:
            # Если сейчас установлено предупреждение, убираем его
            if self.status_var.get().startswith("⚠️"):
                self.status_var.set("Готов к работе")
            return False
    ...
    def on_paste_event(self, event):
        ...
        # Даём время на фактическую вставку, затем проверяем текст
        self.root.after(10, self.after_paste)

    def after_paste(self):
        text = self.entry.get()
        self.update_status_for_text(text)
    ...
    def on_key_release(self, event):
        ...
        self.update_status_for_text(current)
        self.schedule_decode()
    ...
    def on_decode(self, event=None):
        ...
        if self.has_cyrillic(code):
            self.result_text.delete(1.0, tk.END)
            self.result_text.insert(tk.END, "⚠️ Обнаружены русские буквы!\n")
